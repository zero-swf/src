...
package zero.swf.avm1{
	...
	public class ACTIONRECORDs{
		...
		private static const readWriteDoubleBytes:ByteArray=new ByteArray();//20140727
			...
			if(codeOffset<codeEndOffset){
				if(_initByDataOptions&&_initByDataOptions.optimizeAVM1Codes){
						...
						if(AVM1Ops.nameV[code.op]){
							...
							if(code.op<0x80){
								...
								switch(code.op){
									...
									case AVM1Ops.if_://0x9d		//SI16(label)
										...
										if(offsetPos<startPos){
											outputCodeError("解析AVM1代码出错，不合法的偏移量："+(offsetPos-startPos));
										}
									...
									case AVM1Ops.constantPool://0x88	//UI16,STRING[Count]
										if(constStrV){
											outputCodeError("解析AVM1代码出错，发现多个 constCode");
										}
										...
										while(codeOffset<code.pos+3+code.Length){
											switch(codesData[codeOffset++]){
												...
												case 6:
													...
													readWriteDoubleBytes[0]=codesData[codeOffset+3];
													readWriteDoubleBytes[1]=codesData[codeOffset+2];
													readWriteDoubleBytes[2]=codesData[codeOffset+1];
													readWriteDoubleBytes[3]=codesData[codeOffset+0];
													readWriteDoubleBytes[4]=codesData[codeOffset+7];
													readWriteDoubleBytes[5]=codesData[codeOffset+6];
													readWriteDoubleBytes[6]=codesData[codeOffset+5];1
													readWriteDoubleBytes[7]=codesData[codeOffset+4];
													readWriteDoubleBytes.position=0;
													pushValueV[++i]=readWriteDoubleBytes.readDouble();
													codeOffset+=8;
												...
												break;
												default:
													outputCodeError("解析AVM1代码出错，未知 pushType");
												break;
											}
										}
								...
								if(codeOffset==code.pos+3+code.Length){
									...
									outputCodeError("解析AVM1代码出错，"+AVM1Ops.nameV[code.op]+" codeOffset 不正确, codeOffset="+codeOffset+"，code.pos+3+code.Length="+(code.pos+3+code.Length));
					...
					if((labelByPosArr.length-1)>codeEndOffset){
						outputCodeError("解析AVM1代码出错，不合法的偏移量："+((labelByPosArr.length-1)-startPos));
					}
					...
					for(codeOffset=startPos;codeOffset<codeEndOffset;codeOffset++){
						if(labelByPosArr[codeOffset]){
							if(codeByPosArr[codeOffset]){
								...
								outputCodeError("解析AVM1代码出错，不合法的偏移量："+(codeOffset-startPos));
							}
		...
		public function toData(_toDataOptions:Object):ByteArray{
			...
			if(codeArr&&codeArr.length){
				...
				var tooLargePosOffsetObj:Object=null;//20140728
				
				do{
					
					if(tooLargePosOffsetObj){
						
						if(fixTooLargePosOffset(codeArr,tooLargePosOffsetObj.posOffset,tooLargePosOffsetObj.code,tooLargePosOffsetObj.label)){
							tooLargePosOffsetObj=null;
							output("修复超出范围的 posOffset 成功");
						}else{
							output("尝试修复超出范围的 posOffset 失败","red");
							break;
						}
													...
													switch(pushValue.constructor){
														...
														case Number:
															if(pushValue is int){
																...
																readWriteDoubleBytes.position=0;
																readWriteDoubleBytes.writeDouble(pushValue);
																codesData[codeOffset+3]=readWriteDoubleBytes[0];
																codesData[codeOffset+2]=readWriteDoubleBytes[1];
																codesData[codeOffset+1]=readWriteDoubleBytes[2];
																codesData[codeOffset+0]=readWriteDoubleBytes[3];
																codesData[codeOffset+7]=readWriteDoubleBytes[4];
																codesData[codeOffset+6]=readWriteDoubleBytes[5];
																codesData[codeOffset+5]=readWriteDoubleBytes[6];
																codesData[codeOffset+4]=readWriteDoubleBytes[7];
																codeOffset+=8;
						...
						if(labelDict[labelObj.label]){
							...
							if(labelObj.offsetIsSI16){
								if(posOffset>=-0x8000&&posOffset<0x8000){
									...
									output("posOffset="+posOffset+"，超出范围","red");
									tooLargePosOffsetObj={posOffset:posOffset,code:labelObj.code,label:labelObj.label};
				...
				}while(tooLargePosOffsetObj);//20140728
...
function fixTooLargePosOffset(codeArr:Array,posOffset:int,code:Object,label:Object):Boolean{
	
	var id1:int=codeArr.indexOf(code);
	if(id1>-1){
	}else{
		return false;
	}
	
	var id2:int=codeArr.indexOf(label);
	if(id2>-1){
	}else{
		return false;
	}
	
	//var seg:int=Math.ceil(Math.abs(posOffset)/32768);
	var seg:int=Math.ceil(Math.abs(posOffset)/20000);
	var midJumpCodeArr:Array=new Array();
	var jointJumpCodeArr:Array=new Array();
	for(var i:int=0;i<seg-1;i++){
		midJumpCodeArr[i]={op:AVM1Ops.jump,label:{}};
		jointJumpCodeArr[i]={op:AVM1Ops.jump,label:{}};
	}
	
	codeArr[id2]=midJumpCodeArr[midJumpCodeArr.length-1].label;
	
	i=-1;
	for each(var jointJumpCode:Object in jointJumpCodeArr){
		i++;
		codeArr.splice(id1+int((id2-id1)*(i+1)/seg),0,jointJumpCode,i>0?midJumpCodeArr[i-1].label:label,midJumpCodeArr[i],jointJumpCode.label);
	}
