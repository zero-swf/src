	...
	public class DefineBitsLossless{
		...
		public function getImgData(encoder:Object=null):ByteArray{
			if(ZlibBitmapData){
				...
				if(encoder){
					var imgData:ByteArray=encoder.encode(bmd);
				}
				
				if(imgData){
				}else if(bmd.hasOwnProperty("encode")){
					imgData=bmd["encode"](bmd.rect,new (getDefinitionByName("flash.display.PNGEncoderOptions"))());
				}
		...
		//CONFIG::USE_XML{
			...
			public function zhinengToXML(xmlName:String,_toXMLOptions:Object):XML{
				...
				var imgData:ByteArray=getImgData(_toXMLOptions.pngEncoder);
