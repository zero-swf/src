			...
			if(classV){
				...
				if(count){
					...
					for each(var clazz:ABCClass in classV){
						...
						var classXML:XML=clazz.toXML("class",_toXMLOptions);
						
						if(_toXMLOptions.zhinengABCClasses){//20140722
							
							var src:String=_toXMLOptions.saveData("AVM2代码/classes",null,clazz.name.toStr(_toXMLOptions),classXML.toXMLString(),"xml");
							
							childXML.appendChild(<class
								xmlSrc={src}
							/>);
							
						}else{
							
							childXML.appendChild(classXML);
			...
			if(scriptV){
				...
				if(count){
					...
					for each(var script:ABCScript in scriptV){
						...
						var scriptXML:XML=script.toXML("script",_toXMLOptions);
						
						if(_toXMLOptions.zhinengABCClasses){//20140722
							
							if(script.traitV&&script.traitV.length==1){
								var info:String=script.traitV[0].name.toStr(_toXMLOptions)
							}else{
								info=null;
							}
							src=_toXMLOptions.saveData("AVM2代码/scripts",null,info,scriptXML.toXMLString(),"xml");
							
							childXML.appendChild(<script
								xmlSrc={src}
							/>);
							
						}else{
							
							childXML.appendChild(scriptXML);
			...
			if(_toXMLOptions.abcclassesValues.v.length){
				...
				for each(method in _toXMLOptions.abcclassesValues.v){
					...
					if(_toXMLOptions.zhinengABCClasses){//20140722
						...
						src=_toXMLOptions.saveData("AVM2代码/functions",null,null,methodXML.toXMLString(),"xml");
						
						childXML.appendChild(<function
							xmlSrc={src}
						/>);
